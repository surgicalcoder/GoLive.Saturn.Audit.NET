...
public class FileSystemMonitorOptions
{
    ...
    /// <summary>
    /// A function that given a file event, returns true if the entry should be logged and false otherwise. Default includes
    /// all the files.
    /// </summary>
    public Func<FileSystemEventArgs, bool> CustomFilterPredicate { get; set; }

    /// <summary>
    /// A function that given a file info, returns the content type to determine whether the file content should be included
    /// and if it's textual or binary data. Default is not including the content.
    /// </summary>
