...
{
    private static readonly HashSet<string> IgnoredCommands = new(new[] { "isMaster", "buildInfo", "getLastError", "saslStart", "saslContinue" });
    private readonly AuditMongoConfigurator _config = new();

    // RequestId -> CommandStartedEvent
    internal readonly ConcurrentDictionary<int, IAuditScope> _requestBuffer = new();
    ...
    /// </summary>
    public MongoAuditEventSubscriber(Action<IAuditMongoConfigurator> config)
        : this()
    {
        if (config != null)
        {
