...
public class AuditScopeFactory : IAuditScopeFactory
{
    ...
    /// <summary>
    /// Creates an audit scope with the given extra fields, and saves it right away using the globally configured data
    /// provider. Shortcut for CreateAndSave().
    /// </summary>
    ...
    /// <summary>
    /// Creates an audit scope with the given extra fields, and saves it right away using the globally configured data
    /// provider. Shortcut for CreateAndSaveAsync().
    /// </summary>
    /// <param name="eventType">Type of the event.</param>
    /// <param name="extraFields">An anonymous object that can contain additional fields to be merged into the audit event.</param>
    /// <param name="cancellationToken">The Cancellation Token.</param>
    ...
    {
        return new AuditScope(new AuditScopeOptions(eventType, target)).Start();
    }
    ...
    {
        return await new AuditScope(new AuditScopeOptions(eventType, target)).StartAsync(cancellationToken);
    }

    /// <summary>
    /// Creates an audit scope for a target object and an event type, providing the creation policy and optionally the Data
    /// Provider.
    /// </summary>
    /// <param name="eventType">Type of the event.</param>
    /// <param name="target">The reference object getter.</param>
