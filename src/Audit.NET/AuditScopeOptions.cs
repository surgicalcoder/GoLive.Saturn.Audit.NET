...
public class AuditScopeOptions
{
    ...
    /// <summary>
    /// Sets the data provider as a factory method that will be invoked the first time it's needed and only once. This is a
    /// shortcut to set a LazyDataProvider.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets the event creation policy to use. When NULL, it will use the static
    /// Audit.Core.Configuration.CreationPolicy.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets a specific calling method to store on the event. When NULL, it will use the calling stack to determine the
    /// calling method.
    /// </summary>
    public MethodBase CallingMethod { get; set; }

    /// <summary>
    /// Gets or sets a value indicating whether the audit event's environment should include the full stack trace. When NULL,
    /// it will use the static Audit.Core.Configuration.IncludeStackTrace.
    /// </summary>
    public bool? IncludeStackTrace { get; set; }

    /// <summary>
    /// Gets or sets a value indicating whether the audit event's environment should include the timestamps. When NULL, it will
    /// use the static Audit.Core.Configuration.IncludeTimestamps.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets a value indicating whether the audit event should include the Distributed Tracing Activity data. When
    /// NULL, it will use the static Audit.Core.Configuration.IncludeActivityTrace.
    /// </summary>
    ...
    /// When NULL, it will use the static Audit.Core.Configuration.StartActivityTrace.
    /// </summary>
    public bool? StartActivityTrace { get; set; }

    /// <summary>
    /// Gets or sets a value indicating whether the environment information should be excluded from the audit event. When NULL,
    /// it will use the static Audit.Core.Configuration.ExcludeEnvironmentInfo.
    /// </summary>
