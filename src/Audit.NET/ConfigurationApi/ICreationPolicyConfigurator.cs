...
public interface ICreationPolicyConfigurator
{
    ...
    /// <summary>
    /// Use an "Insert On Start - Insert On End" creation policy. The events are inserted when the scope starts, and inserted
    /// again when the scope ends.
    /// </summary>
    IActionConfigurator WithInsertOnStartInsertOnEndCreationPolicy();

    /// <summary>
    /// Use an "Insert On Start - Replace On End" creation policy. The events are inserted when the scope starts, and replaced
    /// when the scope ends.
    /// </summary>
