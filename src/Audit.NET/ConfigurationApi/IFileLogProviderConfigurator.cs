...
public interface IFileLogProviderConfigurator
{
    ...
    /// <summary>
    /// Specifies the directory builder to get the path where to store the audit log files. If this setting is provided,
    /// Directory setting will be ignored.
    /// </summary>
    /// <param name="directoryPathBuilder">The directory path builder. A function that returns the file system path to store
    /// the output for an event.</param>
    ...
    /// </summary>
    /// <param name="filenameBuilder">The filename builder. A function that returns the file name to store the output for an
    /// event.</param>
    /// <returns>IFileLogProviderConfigurator.</returns>
