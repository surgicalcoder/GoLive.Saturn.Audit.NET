...
public interface IAuditScope : IDisposable, IAsyncDisposable
{
    ...
    /// <summary>
    /// Adds a new timed sub-event to the current AuditEvent, associating the specified data and optional custom fields with
    /// the event at the current system time.
    /// </summary>
    ...
    /// <param name="extraFields">An object representing extra custom fields to associate with the timed event. May be null if
    /// no additional fields are required.</param>
    ...
    /// <summary>
    /// Returns the associated Activity instance, or null if no activity is linked to the audit scope (for example, when
    /// StartActivityTrace is disabled).
    /// </summary>
