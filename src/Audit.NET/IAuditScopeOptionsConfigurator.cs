...
public interface IAuditScopeOptionsConfigurator
{
    ...
    /// <summary>
    /// Use a deferred factory to resolve the data provider for each audit event. The factory will be called for each
    /// individual Audit Event to be saved. This is a shortcut to set a DeferredDataProvider.
    /// </summary>
    IAuditScopeOptionsConfigurator DataProviderDeferredFactory(Func<AuditEvent, AuditDataProvider> dataProviderFactory);

    /// <summary>
    /// Sets the data provider factory to use within this scope. The initializer method will be invoked the first time it's
    /// needed and only once. This is a shortcut to set a LazyDataProvider.
    /// </summary>
