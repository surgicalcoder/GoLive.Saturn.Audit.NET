...
/// This data provider does not allow replacing events, the CreationPolicy InsertOnStartReplaceOnEnd is not allowed when
...
public class BlockingCollectionDataProvider : AuditDataProvider
{
    ...
    /// <summary>
    /// Returns a read-only collection of audit events currently stored in memory, this will not remove audit events from the
    /// queue.
    /// </summary>
    ...
    /// <summary>
    /// Takes an audit event from the internal collection and remove it from the queue.
    /// It will block until there is an audit event to be consumed, the timeout is reached or the cancellation token is
    /// triggered.
    ...
    /// The enumerator will continue to provide items (if any exist), otherwise the loop blocks until an item becomes available
    /// or until the CancellationToken is cancelled.
    /// </summary>
