...
/// <summary>
/// A dynamic provider that lets you create a an audit data provider on the fly by specifying the actions to take as lambda
/// expressions.
/// </summary>
public class DynamicAsyncDataProvider : AuditDataProvider
{
    private readonly List<Func<AuditEvent, CancellationToken, Task<object>>> _onInsert = new();
    private readonly List<Func<object, AuditEvent, CancellationToken, Task>> _onReplace = new();

    public DynamicAsyncDataProvider() { }

    public DynamicAsyncDataProvider(Action<IDynamicAsyncDataProviderConfigurator> config)
    {
        var dataProvider = new DynamicAsyncDataProvider();
        var dynConfig = new DynamicAsyncDataProviderConfigurator(dataProvider);
    ...
    {
        _onInsert.Add(async (ev, _) =>
        {
            await insertAction.Invoke(ev);
    ...
    {
        _onInsert.Add(async (ev, ct) =>
        {
            await insertAction.Invoke(ev, ct);
    ...
    /// <summary>
    /// Attaches an action to be executed by the InsertEvent and the ReplaceEvent methods, the first parameter (event id) will
    /// be NULL in case of insert.
    /// The InsertEvent will generate and return a random Guid as the event id.
    /// </summary>
    /// <param name="action">The action.</param>
    ...
    public void AttachOnInsertAndReplace(Func<object, AuditEvent, CancellationToken, Task> action)
    {
        _onInsert.Add(async (ev, ct) =>
        {
            await action.Invoke(null, ev, ct);
