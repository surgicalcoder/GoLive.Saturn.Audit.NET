...
/// <summary>
/// A dynamic provider that lets you create an audit data provider on the fly by specifying the Insert and Replace actions
/// as lambda expressions.
/// </summary>
public class DynamicDataProvider : AuditDataProvider
{
    private readonly List<Func<AuditEvent, object>> _onInsert = new();
    private readonly List<Action<object, AuditEvent>> _onReplace = new();

    public DynamicDataProvider() { }

    public DynamicDataProvider(Action<IDynamicDataProviderConfigurator> config)
    {
        var dataProvider = new DynamicDataProvider();
        var dynConfig = new DynamicDataProviderConfigurator(dataProvider);
