...
public readonly struct Setting<T>
{
    ...
    /// If the setting was created with a value builder, the default value corresponds to the outcome of the value builder when
    /// provided with a null AuditEvent.<br />
    ...
    public static implicit operator Setting<T>(Func<AuditEvent, T> func)
    {
        return new Setting<T>(func);
    }
}